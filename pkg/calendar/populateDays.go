...
func (d Date) PopulateDays(containerObj *fyne.Container, date time.Time) {
	...
	for day := 1; day <= lastDay.Day(); day++ {
		dayButton := widget.NewButton(
			...
			func() {
				d.OnSelected(time.Date(date.Year(), date.Month(), day, 0, 0, 0, 0, time.UTC))
			},
		)
		dayStr := fmt.Sprintf("%d-%02d-%02d", date.Year(), date.Month(), day)
		var border *canvas.Rectangle
		border = canvas.NewRectangle(color.Transparent)
		if d.SpecialDays[dayStr] {
			border = canvas.NewRectangle(color.RGBA{R: 255, G: 255, B: 0, A: 255})
			border.StrokeColor = color.RGBA{R: 255, G: 0, B: 0, A: 255}
			border.FillColor = color.RGBA{R: 255, G: 255, B: 0, A: 255}
